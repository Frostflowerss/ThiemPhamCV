  ...
  return (
    <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="rounded-2xl border border-[color:var(--line)] bg-[color:var(--card)] p-6">
        <Reveal>
          <div className="text-sm font-semibold tracking-tight">Học vấn</div>
        </Reveal>

        <div className="mt-4 space-y-4">
          {education.map((e) => (
            <div key={e.school}>
              <div className="text-xs text-[color:var(--muted)]">{e.period} • {e.location}</div>
              <div className="mt-1 text-sm font-medium">{e.school}</div>
      ...
      <div className="rounded-2xl border border-[color:var(--line)] bg-[color:var(--card)] p-6">
        <Reveal>
          <div className="text-sm font-semibold tracking-tight">Khoá học</div>
        </Reveal>

        <div className="mt-4 space-y-4">
          {courses.map((c) => (
            <div key={c.title + c.year}>
              <div className="text-xs text-[color:var(--muted)]">{c.year} • {c.location}</div>
